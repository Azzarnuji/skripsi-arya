				...
				<tbody class="table-border-bottom-0">
					...
					<?php foreach ($users as $user) : ?>
						<tr>
							...
							<td><?= $user['no_sim'] ?></td>
							<td>
								<a class="btn btn-sm btn-secondary text-white" data-bs-toggle="modal" data-bs-target="#modalViewAndEdit" data-bs-noktp="<?= $user['no_ktp']; ?>" data-bs-type="edit">
									Edit
								</a>
								<a data-bs-toggle="modal" data-bs-target="#modalViewAndEdit" data-bs-noktp="<?= $user['no_ktp']; ?>" data-bs-type="view" class="btn btn-sm btn-primary text-white">
									Lihat
								</a>
								<button class="btn btn-sm btn-warning" onclick="window.askBeforeExecution('Yakin Hapus Data ?',()=>{
									window.location.href = '<?= base_url() ?>/admin/hapusSupir/' + '<?= $user['no_ktp'] ?>';
								})">
									Hapus
								</button>
							</td>
